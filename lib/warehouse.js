...
var warehouse = module.exports = {
  ...
  _populateWarehouseForRelease: function(releaseVersion) {
    ...
    if (engineVersion !== files.getEngineVersion()) {
      // xcxc make this comment nicer. refer to the upcoming node stream refactor
      // ARGH memory consumption. engines are *big* (>100MB uncompressed)
      try {
        // XXX this sucks. We store 
        engineTarball = Future.wrap(files.getUrl)({
          url: PACKAGES_URLBASE + "/engines/" + releaseManifest.engine + ".", // xcxc correct filename
          encoding: null
        }).wait();
