...
///
/// SIMULATE RUNNING FROM AN INSTALLED VERSION
///
var origInCheckout = files.in_checkout;
files.in_checkout = function () {
  return false;
};
...
assert.doesNotThrow(function () {
  ...
  // xcxc so we need to pass release everywhere now? all apps are versioned?
...
console.log("unversioned app, using `release`");
assert.doesNotThrow(function () {
  var tmpOutputDir = tmpDir();
  var errors = bundler.bundle(unversionedAppDir, tmpOutputDir, {release: '0.0.xcxc', nodeModulesMode: 'skip'});
  assert.strictEqual(errors, undefined);
...
// revert stub
files.in_checkout = origInCheckout;